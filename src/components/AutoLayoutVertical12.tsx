...
export type AutoLayoutVertical12Type = {
  className?: string;
};

const AutoLayoutVertical12: FunctionComponent<AutoLayoutVertical12Type> = ({
  className = "",
}) => {
  return (
    <div
      className={`self-stretch rounded-t-radius-28 rounded-b-none bg-[#171717] overflow-hidden flex flex-col items-center justify-start p-spacing-spacing-32 gap-spacing-spacing-40 z-[1] mt-[-396px] relative text-left text-[26px] text-[#fff] font-Web-title-1-normal ${className}`}
    >
      <div className="self-stretch flex flex-col items-center justify-start gap-3.5">
        <h2 className="m-0 self-stretch relative text-[length:inherit] tracking-[-0.2px] leading-[32px] font-semibold font-[inherit]">
          ...
          <span className="text-[#fff142]">confidence</span>
          <span>{`, `}</span>
          <span className="text-[#fff142]">hassle-free</span>
        </h2>
        <div className="self-stretch relative text-font-size-title-2 tracking-[-0.2px] leading-font-line-height-title-2 font-medium text-[#bdbdbd]">
          Join opportunities selected by experts and start earning even if it's
      ...
      <StepperDot11 size="Small" state="1st Active" />
    </div>
  );
};

export default AutoLayoutVertical12;
